...
type ModalState = { kind: "summary"; text: string } | { kind: "rewrite"; before: string; after: string } | null;
...
export function AiPanel({ documentId, canEdit, online, currentContent, onApplyRewrite, onApplyTitle }: AiPanelProps) {
  ...
  async function runAction(action: AiAction) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        setError(formatRetryMessage(Number.isFinite(retryAfterSec) ? retryAfterSec : null));
        return;
      }
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        title={!online ? "AI requires network" : undefined}
      ...
      {error ? (
        <p className="absolute right-0 top-full z-30 mt-2 w-72 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900 shadow">
          {error}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" className="btn btn-secondary" onClick={() => void copySummary(modal.text)}>
                {copied ? "Copied" : "Copy"}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative flex max-h-[85vh] w-full max-w-4xl flex-col rounded-2xl bg-white shadow-2xl">
            <header className="border-b border-zinc-200 px-5 py-4">
              <h2 className="text-lg font-semibold text-zinc-900">Rewrite preview</h2>
              <p className="mt-0.5 text-xs text-zinc-500">Review the AI rewrite, then apply it through normal sync.</p>
            </header>
            <div className="grid min-h-0 flex-1 gap-4 overflow-hidden p-5 md:grid-cols-2">
              <div className="min-h-0 overflow-auto rounded-xl border border-zinc-200 bg-zinc-50 p-3">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">Current</p>
                <pre className="whitespace-pre-wrap text-sm leading-6 text-zinc-700">{modal.before || "(empty)"}</pre>
              </div>
              <div className="min-h-0 overflow-auto rounded-xl border border-[#c2d7fc] bg-[#f8fbff] p-3">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#1557b0]">Rewrite</p>
                <pre className="whitespace-pre-wrap text-sm leading-6 text-zinc-800">{modal.after}</pre>
              </div>
            </div>
